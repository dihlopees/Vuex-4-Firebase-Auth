<template>
  <div class="home">
    <p> Testando alteração de estado com store: {{point}}  </p>

    <button @click="updatePoints(2)"> add 2 points</button>
...
import populateStorage from "../store/local"
...
export default {
  setup() {
    ...
    const point = computed(() => store.state.points)

    const updatePoints = (payload) => {
      store.commit('updatePoints' , payload)
      populateStorage()
    }


    return { 
      blogs, point, updatePoints
    }
  }
}
</script>
